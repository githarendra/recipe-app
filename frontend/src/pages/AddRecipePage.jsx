  ...
  return (
    <div className="max-w-2xl mx-auto px-8 py-10">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
        <h1 className="text-2xl font-semibold mb-8 text-zinc-900 dark:text-zinc-100">
          Add a recipe
        </h1>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          {/* Cooking time, servings, category */}
                ...
                <div key={i} className="flex gap-2">
                  ...
                  {ingredients.length > 1 && (
                    <button type="button" onClick={() => removeIngredient(i)}
                      className="px-3 rounded-xl border border-zinc-200 dark:border-zinc-700 text-zinc-400 hover:text-red-500 hover:border-red-300 transition-colors text-sm">
                      ✕
                    </button>
                  )}
                </div>
            ...
            <div className="space-y-2">
              {steps.map((step, i) => (
                <div key={i} className="flex gap-2 items-start">
                  <span className="w-6 h-6 mt-2.5 rounded-full bg-orange-500 text-white flex items-center justify-center text-xs font-medium shrink-0">
                    {i + 1}
                  </span>
                  <textarea
                  ...
                  {steps.length > 1 && (
                    <button type="button" onClick={() => removeStep(i)}
                      className="px-3 py-2.5 rounded-xl border border-zinc-200 dark:border-zinc-700 text-zinc-400 hover:text-red-500 hover:border-red-300 transition-colors text-sm">
                      ✕
                    </button>
                  )}
                </div>
